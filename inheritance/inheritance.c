...
person *create_family(int generations)
{
    //Allocae memort for a new person
    ...
    if (generations > 1)
    {
        ...
        //setting the parent pointers for the current person
        newPerson -> parents[0] = parent0;
        newPerson -> parents[1] = parent1;

        //Randomly assign the current person's alleles based of parents
        newPerson  -> alleles[0] = parent0 -> alleles[rand() % 2];
        newPerson  -> alleles[1] = parent1 -> alleles[rand() % 2];
    }
    else
    {
        //set parent pointers to NULL for last generation
